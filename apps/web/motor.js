...
var Motor = (() => {
  ...
  __export(index_exports, {
    AVISO_SIMULACAO: () => AVISO_SIMULACAO,
    Decimal: () => decimal_default,
    ErroApuracao: () => ErroApuracao,
    PARAMETROS_BR_2026: () => PARAMETROS_BR_2026,
    apurar: () => apurar,
    impostoProgressivo: () => impostoProgressivo,
    simularAnualExterior: () => simularAnualExterior,
    simularMensalNacional: () => simularMensalNacional,
    verificarInvariantes: () => verificarInvariantes
  });
  ...
  var ErroApuracao = class extends Error {
    constructor(message, lancamentoId, codigo) {
      super(message);
      __publicField(this, "lancamentoId", lancamentoId);
  ...
  // src/posicoes.ts
  var d = (v, padrao = "0") => new decimal_default(v === void 0 || v === null || v === "" ? padrao : v);
  ...
  var POEIRA = new decimal_default("1e-18");
  function paraBrl(l, valor) {
    if (l.moeda === "BRL") return valor;
    const cambio = d(l.cambioBrl);
    if (cambio.lte(0)) {
      throw new ErroApuracao(
        `Lan\xE7amento ${l.id} est\xE1 em ${l.moeda} mas n\xE3o tem c\xE2mbio da data.`,
        l.id,
        "CAMBIO_AUSENTE"
      );
    }
  ...
  function apurar(lancamentos) {
    ...
    for (const l of ordenados) {
      ...
      if (qtd.lte(0)) {
        throw new ErroApuracao(`Lan\xE7amento ${l.id} tem quantidade n\xE3o positiva.`, l.id, "QTD_INVALIDA");
      }
      ...
      switch (l.tipo) {
        ...
        case "airdrop":
        case "bonificacao": {
          if (l.precoUnitario === void 0) {
            throw new ErroApuracao(
              `Lan\xE7amento ${l.id} (${l.tipo}) precisa de pre\xE7o unit\xE1rio.`,
              l.id,
              "PRECO_AUSENTE"
            );
          }
        ...
        case "permuta_saida": {
          ...
          if (qtd.gt(a.quantidade.plus(POEIRA))) {
            throw new ErroApuracao(
              `Lan\xE7amento ${l.id} aliena ${qtd} de ${l.ativoId}, mas o saldo \xE9 ${a.quantidade}. Falta registrar aquisi\xE7\xF5es anteriores.`,
              l.id,
              "SALDO_INSUFICIENTE"
            );
          }
          ...
          if (incerto) {
            avisos.push(
              `A aliena\xE7\xE3o ${l.id} consumiu custo n\xE3o documentado. O ganho apurado \xE9 estimativa.`
            );
          }
          break;
        }
        ...
        case "perda": {
          if (qtd.gt(a.quantidade.plus(POEIRA))) {
            throw new ErroApuracao(
              `Lan\xE7amento ${l.id} baixa ${qtd} de ${l.ativoId}, mas o saldo \xE9 ${a.quantidade}.`,
              l.id,
              "SALDO_INSUFICIENTE"
            );
          }
    ...
    const posicoes = [...contas.values()].map((a) => {
  ...
  function verificarInvariantes(r) {
    const falhas = [];
    for (const p of r.posicoes) {
      ...
      if (p.quantidade.gt(0) && p.custoMedioBrl.lt(0)) {
        falhas.push(`${p.ativoId}: custo m\xE9dio negativo`);
      }
      const soma = p.confianca.documentado.plus(p.confianca.estimado).plus(p.confianca.arbitrado).plus(p.confianca.ausente);
      if (!soma.minus(p.custoTotalBrl).abs().lt("1e-12")) {
        falhas.push(`${p.ativoId}: perfil de confian\xE7a n\xE3o fecha com o custo total`);
      }
    }
    return falhas;
  }
  ...
  var PARAMETROS_BR_2026 = {
    isencaoMensalNacional: new decimal_default("35000"),
    faixasGanhoCapital: [
      { ate: new decimal_default("5000000"), aliquota: new decimal_default("0.15") },
      { ate: new decimal_default("10000000"), aliquota: new decimal_default("0.175") },
      { ate: new decimal_default("30000000"), aliquota: new decimal_default("0.20") },
      { ate: new decimal_default(Infinity), aliquota: new decimal_default("0.225") }
    ],
    aliquotaExterior: new decimal_default("0.15"),
    compensarPerdasNoMesNacional: false,
    vigenciaDescrita: "Par\xE2metros conforme leitura das regras vigentes em agosto de 2026"
  };
  function impostoProgressivo(base, p) {
    if (base.lte(0)) return ZERO2;
    let imposto = ZERO2;
    let piso = ZERO2;
    for (const faixa of p.faixasGanhoCapital) {
      if (base.lte(piso)) break;
      const topo = decimal_default.min(base, faixa.ate);
      imposto = imposto.plus(topo.minus(piso).times(faixa.aliquota));
      piso = faixa.ate;
    }
    return imposto;
  }
  var competenciaDe = (iso) => iso.slice(0, 7);
  function simularMensalNacional(alienacoes, p = PARAMETROS_BR_2026) {
    ...
    return [...porMes.entries()].sort(([x], [y]) => x.localeCompare(y)).map(([competencia, lista]) => {
      const totalAlienacoes = lista.reduce((s, a) => s.plus(a.valorAlienacaoBrl), ZERO2);
      const ganhoBruto = lista.reduce(
        (s, a) => a.ganhoBrl.gt(0) ? s.plus(a.ganhoBrl) : s,
        ZERO2
      );
      const resultadoLiquido = lista.reduce((s, a) => s.plus(a.ganhoBrl), ZERO2);
      ...
      const baseCalculo = isento || baseBruta.lte(0) ? ZERO2 : baseBruta;
      ...
      return {
        ...
        isento,
        motivoIsencao: isento ? `Aliena\xE7\xF5es do m\xEAs (R$ ${totalAlienacoes.toFixed(2)}) dentro do limite de R$ ${p.isencaoMensalNacional.toFixed(2)}` : void 0,
        baseCalculoBrl: baseCalculo,
        impostoBrl: imposto,
        aliquotaEfetiva: baseCalculo.isZero() ? ZERO2 : imposto.div(baseCalculo),
        alienacoes: lista
      };
    });
  }
  function simularAnualExterior(alienacoes, p = PARAMETROS_BR_2026) {
    ...
    return [...porAno.entries()].sort(([x], [y]) => x - y).map(([ano, lista]) => {
      const total = lista.reduce((s, a) => s.plus(a.valorAlienacaoBrl), ZERO2);
      const resultado = lista.reduce((s, a) => s.plus(a.ganhoBrl), ZERO2);
      const base = decimal_default.max(resultado, ZERO2);
      return {
        ano,
        regime: "exterior",
  ...
  var AVISO_SIMULACAO = "Este relat\xF3rio \xE9 uma SIMULA\xC7\xC3O gerada automaticamente a partir dos lan\xE7amentos informados. N\xE3o constitui apura\xE7\xE3o fiscal, orienta\xE7\xE3o tribut\xE1ria ou recomenda\xE7\xE3o de investimento. Os valores dependem da exatid\xE3o e da completude dos dados inseridos e da interpreta\xE7\xE3o da legisla\xE7\xE3o vigente. Confira com contador ou advogado tributarista habilitado antes de declarar ou recolher qualquer tributo.";
